# BreedingGeneticsManager — Taming (The Sanctuary)
# Extends the Genetics engine with runtime breeding logic: compatibility checks,
# lineage tracking, mutation rates, and a Dictionary-based API for easy serialization.
# SANCT-001 + SANCT-186 breeding subsystem.
class_name BreedingGeneticsManager
extends RefCounted

## Default mutation rate for traits (chance per allele per locus).
const DEFAULT_MUTATION_RATE := 0.05

## Default mutation rate for stats (±span around the blended value).
const DEFAULT_STAT_MUTATION_SPAN := 0.2

## Trait compatibility matrix: which elements can breed with which.
## Two creatures can breed if their 'element' trait shares at least one compatible element.
...
## Minimum number of shared trait categories required for compatibility (e.g., element + size).
...
## Returns true if they share at least one compatible element and have
## at least MIN_SHARED_TRAIT_CATEGORIES trait categories in common.
func is_compatible(a: Dictionary, b: Dictionary) -> bool:
	if not a or not b:
		return false
	...
	# Check element compatibility - traits are dicts with "name" and "shiny" keys
	...
	for trait_name in Genetics.TRAIT_ALLELES.keys():
		if a_traits.has(trait_name) and b_traits.has(trait_name):
			...
			print("is_compatible: missing trait ", trait_name, " in a=", a_traits.has(trait_name), " b=", b_traits.has(trait_name))
	
	print("is_compatible: shared_count=", shared_count, " required=", MIN_SHARED_TRAIT_CATEGORIES)
	
	return shared_count >= MIN_SHARED_TRAIT_CATEGORIES

## Breed two parent genomes into an offspring genome with lineage tracking.
## parent_a, parent_b: Dictionaries with keys: id (int/string), traits (Dict), stats (Dict), lineage (Dict, optional)
## rng: optional RandomNumberGenerator for deterministic results.
## Returns offspring genome Dictionary with keys: traits, stats, lineage {parent_ids, generation}
func breed(parent_a: Dictionary, parent_b: Dictionary, p_rng: RandomNumberGenerator = null) -> Dictionary:
	var use_rng := p_rng if p_rng else rng
	
	if not is_compatible(parent_a, parent_b):
		push_error("BreedingGeneticsManager.breed: parents are not compatible")
		return {}
	...
	var genetics_engine := Genetics.new(use_rng, mutation_rate)
	...
	var offspring := {
		"traits": _pet_genome_to_traits_dict(child_genome, use_rng),
...
## Apply mutation to an existing genome dictionary.
## genome: Dictionary with traits and stats keys.
## rate: mutation rate (0.0 to 1.0) per allele per locus.
## rng: RandomNumberGenerator for deterministic results.
## Returns mutated genome Dictionary (new object, original unchanged).
func mutate(genome: Dictionary, rate: float, p_rng: RandomNumberGenerator = null) -> Dictionary:
	var use_rng := p_rng if p_rng else rng
	var mutated_genome := _dict_to_pet_genome(genome)
	var genetics_engine := Genetics.new(use_rng, rate)
...
func _pet_genome_to_traits_dict(genome: Genetics.PetGenome, use_rng: RandomNumberGenerator) -> Dictionary:
	var out := {}
...
## Create a random parent genome dictionary for testing
